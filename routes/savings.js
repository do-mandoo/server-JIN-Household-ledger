...
// JSON 데이터 파일 경로
const DATA_FILE = path.join(__dirname, '../data/savings.json');
  ...
  fs.readFile(DATA_FILE, 'utf8', (err, data) => {
    ...
    fs.writeFile(DATA_FILE, JSON.stringify(jsonData, null, 2), err => {
      if (err) {
        ...
        res.status(201).json(newEntry); // 생성된 데이터 반환
      }
    });
  });
});
...
router.put('/:id', (req, res) => {
  ...
  const updatedEntry = req.body; // 클라이언트에서 보낸 데이터
  ...
  fs.readFile(DATA_FILE, 'utf8', (err, data) => {
    ...
    // 기존 데이터를 유지하면서 새로운 데이터 병합
    jsonData[index] = {
      ...jsonData[index], // 기존 데이터
      ...updatedEntry, // 업데이트된 데이터
    };
    ...
    fs.writeFile(DATA_FILE, JSON.stringify(jsonData, null, 2), err => {
      if (err) {
        ...
        res.json(jsonData[index]); // 업데이트된 데이터 반환
      }
    });
  });
});
